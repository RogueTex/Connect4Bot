...
def main():
    ...
    for e in events:
        if e.get("event_type") == "game_end" and e.get("game_id"):
            results_by_game[str(e["game_id"])] = str(e.get("result", "unknown")).lower()
    ...
    for e in events:
        ...
        if args.keep_model and model_type != args.keep_model.lower():
            continue
        board = e.get("board")
        ...
        result = results_by_game.get(game_id, "unknown")
        if result == "win":
            sw = args.win_weight
        elif result == "loss":
            sw = args.loss_weight
